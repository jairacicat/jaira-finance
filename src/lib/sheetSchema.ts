...
export function yearTab(year: number): string {
  return String(year);
}
export function dashboardTab(year: number): string {
...
async function getSheets(spreadsheetId: string): Promise<SheetMeta[]> {
  const meta = await getSpreadsheetMeta(spreadsheetId);
  return (meta.sheets || []).map((s: any) => ({ sheetId: s.properties.sheetId, title: s.properties.title }));
}

async function ensureSheet(spreadsheetId: string, title: string, existing: SheetMeta[]): Promise<SheetMeta> {
  ...
  const meta: SheetMeta = { sheetId: props.sheetId, title: props.title };
  existing.push(meta);
  return meta;
}

async function ensureHeaders(spreadsheetId: string, title: string, headers: readonly string[]) {
...
export async function bootstrapSpreadsheet(spreadsheetId: string, year: number) {
  const sheets = await getSheets(spreadsheetId);

  // Base tabs
  for (const t of Object.values(TAB)) await ensureSheet(spreadsheetId, t, sheets);
  for (const t of Object.values(TAB)) {
    const headers = HEADERS[t as keyof typeof HEADERS];
...
export async function ensureYear(spreadsheetId: string, year: number) {
  const sheets = await getSheets(spreadsheetId);
  const yt = yearTab(year);
...
async function maybeSeedDefaults(spreadsheetId: string) {
