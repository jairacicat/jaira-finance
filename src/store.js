...
import { appendValues, batchUpdate, clearRange, getValues, getSpreadsheetMeta, getUserEmail, isConfigured, setValues, signIn, signOut } from './lib/google';
import { bootstrapSpreadsheet, ensureYear, HEADERS, TAB, writeDashboardFormulas, yearTab } from './lib/sheetSchema';
import { accountToRow, accountTypeToRow, budgetToRow, categoryToRow, recurringToRow, rowToAccount, rowToAccountType, rowToBudget, rowToCategory, rowToRecurring, rowToTxn, txnToRow } from './lib/mappers';
...
async function upsertSimple(get, set, tab, key, item, toRow) {
    const { settings } = get();
    const list = get()[key].slice();
    const idx = list.findIndex((x) => x.id === item.id);
    const finalItem = item.id ? item : { ...item, id: uid(`${key.slice(0, 3)}_`) };
    if (idx >= 0) {
        list[idx] = finalItem;
        const row = idx + 2;
...
async function deleteSimple(get, set, tab, key, id, numCols) {
    const { settings } = get();
    const list = get()[key].slice();
    ...
    if (list.length > 0) {
        ...
        await setValues(settings.spreadsheetId, `${tab}!A2`, list.map((x) => toRowFn(x)));
    }
    set({ [key]: list });
}
const ROW_FNS = {
...
export async function fetchSpreadsheetTitle(id) {
...
export async function batchUpdateProxy(spreadsheetId, requests) {
